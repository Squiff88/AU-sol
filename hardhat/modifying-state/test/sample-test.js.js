...
describe("TestModifyVariable", function () {
  it("should change x to 1337", async function () {
    ...
    const ModifyVariable = await ethers.getContractFactory("ModifyState");

    // we then use the ContractFactory object to deploy an instance of the contract
    const contract = await ModifyVariable.deploy(10, "Hi There !");
  ...
  it("should change the greet message", async function () {
    // this line creates an ethers ContractFactory abstraction: https://docs.ethers.org/v5/api/contract/contract-factory/
    const ModifyVariable = await ethers.getContractFactory("ModifyState");
    ...
    // modify greet message from the constructor value to new value
    await contract.modifyGreet("Howdy partner !");
    const newGreetMsg = await contract.greet();
    assert.equal(newGreetMsg.toString(), "Howdy partner !");
  });
});
